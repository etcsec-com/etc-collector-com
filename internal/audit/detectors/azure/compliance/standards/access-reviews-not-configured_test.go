...
func TestAccessReviewsNotConfigured_InfoFindingWhenNotProbed(t *testing.T) {
	d := NewAccessReviewsNotConfiguredDetector()
	data := &audit.DetectorData{} // AzureAccessReviewsProbed false — couldn't check
...
func TestAccessReviewsNotConfigured_SilentWhenConfigured(t *testing.T) {
	d := NewAccessReviewsNotConfiguredDetector()
	data := &audit.DetectorData{AzureAccessReviewsProbed: true, AzureAccessReviewsCount: 4}

	if findings := d.Detect(context.Background(), data); len(findings) != 0 {
		t.Fatalf("expected no finding when access reviews exist, got %+v", findings)
	}
}

func TestAccessReviewsNotConfigured_FiresWhenProbedAndEmpty(t *testing.T) {
	d := NewAccessReviewsNotConfiguredDetector()
